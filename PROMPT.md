...
You are implementing this repository. It is a tool called weft: a verification gate for coding agents that checks the wiring, not the spelling. It reconstructs the relationships in a codebase (route to handler, env var to declaration, import to lockfile, and more) and rejects code that references something that does not resolve, before that code ships.
...
That creates a virtual environment, installs the package with all extras and dev tools, and runs the self-test. If the script is missing or fails, do the equivalent by hand: create a venv, `pip install -e ".[all,dev]"`, then `python -m weft.selftest`. Confirm the self-test passes before you change anything.
