...
*How to fold the "graph the repo plus a second brain plus token optimization" idea into weftgate without walking into a red ocean or losing the wedge. Slots into `docs/DESIGN.md` after section 13.*
...
```
     ...
     (weftgate gate)             (grounded slices)          (verified memory)
...
Two consequences for the design. First, every context response is bounded and ranked, and returns pointers and compact cards, never file bodies. A server that returns blobs makes token usage worse, not better. Second, the differentiated token payoff is not retrieval compression, which is a saturated benchmark war, but avoided retry loops. A hallucinated reference that ships costs a full failed round trip, the attempt plus the error plus the correction, which is larger than any single retrieval saving. weftgate prevents that. Measure that.
...
- Every response is capped by a token budget (`WEFTGATE_CONTEXT_BUDGET`, default small). Over budget, return the highest-ranked items plus a count of what was elided, never a truncated blob.
...
Scope guard: this is grounded context, not general semantic search. It does not try to beat the retrieval-graph incumbents on breadth or embeddings. It serves the facts weftgate can verify, served small. If a user wants broad semantic search, they run one of the existing tools alongside; weftgate integrates, it does not reimplement.
...
Concrete hook: the store records, per memory, the referenced node ids and the `valid_commit`. The graph's `sync` emits a changed-node set. A memory whose referenced nodes intersect the changed set flips to `stale` and drops out of recall until re-verified. This reuses weftgate's oracles as the re-verification path: a stale memory is re-checked by the same gate that would check new code.

## A6. The honest token metric

Ship a ledger, but measure the metric you own, not the one you cannot win.

- Record, per session, the context weftgate served small (resolved facts and cards) and, where the agent cooperates, an estimate of the whole-file reads it displaced. Report this as an estimate, labeled as such.
...
- Do not report a single headline compression percentage as if it were a field number. Any figure measured on the same repo it was tuned on is an upper bound, and the docs say so. The credible number is the one a user generates on their own repo with `weftgate audit` and the ledger, not a marketing percentage.
