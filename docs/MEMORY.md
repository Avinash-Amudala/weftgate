...
rather than a memory engine of its own. weft owns the graph and the truth about it;
mnemo owns the memories and their ranking. The seam between them is three small,
stable calls plus a plugin entry point.

## The mechanism

1. **Anchors.** When a memory is stored, mnemo asks weft to ground it:
   `weft memory anchor '{"text": "...", "claims": {"env": [...], "routes": [...],
...
2. **The changed-node ledger.** Every `Session.sync()` snapshots the node sets a sync
   ...
   `weft memory changes --since N` returns what changed after `N`.
...
4. **Re-verification.** `store.py reverify` sends stale anchors back through
   `weft memory check`. If every node still resolves the memory returns to `valid`
...
5. **The gate at write time.** `weft.memory.register(api)` is a mnemo verification
   plugin. With `"oracles": ["weft.memory"]` in `.mnemo.json`, a memory that claims
...
|---|---|---|
| `weft memory anchor <json>` | `memory_anchor` | anchors for a memory's text and claims |
| `weft memory check <json>` | `memory_check` | re-derive anchor states; `summary` is the memory's state |
| `weft memory changes --since N` | `memory_changes` | changed nodes after sequence `N` |

Both surfaces call the same functions in `weft/memory.py`, and a test asserts they agree.

## Try it

```bash
pip install git+https://github.com/Avinash-Amudala/weft.git
cd /path/to/mnemo && python3 mnemo/selftest.py     # the grounding stage runs when weft is importable
```
...
valid again. Without weft the stage skips and says so.
