#!/usr/bin/env bash
# Rename package, config, imports, CLI, entry points, and documentation.
# Run before publishing a distribution: bash scripts/rename.sh <newname>
set -euo pipefail
cd "$(dirname "$0")/.."
python3 - "$@" <<'PY'
import pathlib
...
count = 0
for rel in files:
    if not rel or rel == "scripts/rename.sh":
        continue
    if rel == old + ".toml":
        rel = new + ".toml"
...
print(f"Run lint, types, selftest, and pytest before committing or renaming the remote.")
PY
