...
from weft.config import Config, _parse_toml_subset, detect_stack, find_repo_root, load_toml
...
def test_repo_toml_then_env_override(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("WEFT_ORACLES", "imports_lockfile, env_vars")
    monkeypatch.setenv("WEFT_BLOCK_ON", "never")
    cfg = Config.load(str(tmp_path))
...
def test_toml_subset_parser_matches_tomllib():
    ...
    assert subset["weft"]["oracles"] == ["a", "b"]
    assert subset["weft"]["n"] == 3 and subset["weft"]["flag"] is True
    assert subset["weft"]["routes_fastapi"]["app"] == "x:app"
