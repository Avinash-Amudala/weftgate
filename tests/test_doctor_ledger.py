"""weft doctor explains the setup; the ledger counts blocked changes; index --show dumps."""

import json
import os

from tests.conftest import write
from weft import doctor, ledger
...
def test_doctor_on_the_fixture_and_on_an_empty_dir(tmp_path, capsys):
    ...
    assert "weft setup --hooks" in names["hooks"]["hint"]
    assert cli_main(["--repo", root, "--store", store, "doctor"]) == 0
    out = capsys.readouterr().out
    assert "weft doctor" in out and "everything the gate needs is in place" in out
...
def test_ledger_records_only_blocking_verdicts(tmp_path, capsys, monkeypatch):
    ...
    monkeypatch.setenv("WEFT_LEDGER", "0")
    assert cli_main(["--repo", root, "--store", store, "check", "app/broken.py"]) == 1
