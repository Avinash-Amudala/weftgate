...
def test_no_evidence_is_not_observed_never_proven():
    for kind in ("tests_pass", "endpoint_status", "bug_fixed", "made_up"):
        v = grade(OutcomeClaim(kind, {"command": "pytest", "url": "http://localhost:1/",
                                      "signature": "boom"}))
...
def test_junit_report_can_prove_or_contradict(tmp_path):
    good = write(str(tmp_path), "good.xml",
                 '<testsuites><testsuite tests="3" failures="0" errors="0"/></testsuites>')
    bad = write(str(tmp_path), "bad.xml",
    ...
    assert grade(OutcomeClaim("tests_pass", {"report": str(tmp_path / "notxml.xml")})
...
def test_endpoint_probe_and_self_report():
    ...
    try:
        ...
        assert grade(OutcomeClaim("endpoint_status", {"url": url, "status": 204}),
                     Policy(run=True)).verdict is Honesty.PROVEN
        ...
        assert grade(OutcomeClaim("endpoint_status", {"url": f"http://127.0.0.1:{port}/x",
                                                      "status": 404}),
                     Policy(run=True)).verdict is Honesty.PROVEN
    finally:
        server.shutdown()
    ...
    v = grade(OutcomeClaim("endpoint_status", {"url": "https://example.com/", "status": 200}),
              Policy(run=True))
    assert v.verdict is Honesty.NOT_OBSERVED and "allowlisted" in v.reason
    # A closed local port is a failed observation, not a contradiction.
    v = grade(OutcomeClaim("endpoint_status", {"url": "http://127.0.0.1:1/", "status": 200}),
              Policy(run=True))
    assert v.verdict is Honesty.NOT_OBSERVED
...
def test_bug_fixed_requires_the_observed_signature():
    ...
    v = grade(OutcomeClaim("bug_fixed", {"signature": sig},
                           {"before": f"Traceback...\n{sig}\n", "after": "all good\n"}))
    assert v.verdict is Honesty.PLAUSIBLE
    v = grade(OutcomeClaim("bug_fixed", {"signature": sig},
                           {"before": f"{sig}\n", "after": f"still: {sig}\n"}))
    assert v.verdict is Honesty.CONTRADICTED
    v = grade(OutcomeClaim("bug_fixed", {"signature": sig},
                           {"before": "a different error\n", "after": "ok\n"}))
    assert v.verdict is Honesty.NOT_OBSERVED
    assert grade(OutcomeClaim("bug_fixed", {})).verdict is Honesty.NOT_OBSERVED
    fixed = f"{PY} -c \"print('fine')\""
    still = f"{PY} -c \"print('{sig}')\""
    assert grade(OutcomeClaim("bug_fixed", {"signature": sig, "command": fixed}),
                 _policy(run=True)).verdict is Honesty.PROVEN
    assert grade(OutcomeClaim("bug_fixed", {"signature": sig, "command": still}),
                 _policy(run=True)).verdict is Honesty.CONTRADICTED
    other = f"{PY} -c \"import sys; sys.exit(2)\""
    assert grade(OutcomeClaim("bug_fixed", {"signature": sig, "command": other}),
                 _policy(run=True)).verdict is Honesty.PLAUSIBLE
...
def test_claim_mode_wiring_and_surfaces(tmp_path, capsys):
    ...
    code = cli_main(["--repo", root, "--store", store, "--format", "json", "claim", "--run",
                     json.dumps(claims)])
